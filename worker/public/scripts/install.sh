...
# Map logical package name -> distro-specific package(s).
# May emit multiple space-separated packages (see pkg_install word-split).
resolve_pkg() {
  case "$1:$PKG_MGR" in
    poppler:pacman|poppler:brew) printf "poppler" ;;
    poppler:*)                   printf "poppler-utils" ;;
    gcc:apk)                     printf "gcc musl-dev" ;;  # cgo on musl needs headers
    *)                           printf "%s" "$1" ;;
  esac
}
...
pkg_install() {
  local logical pkgs=()
  # Unquoted on purpose: resolve_pkg may return multiple packages (e.g. gcc musl-dev).
...
install_go() {
  ...
  version="$(curl -fsSL 'https://go.dev/VERSION?m=text' | head -n 1)" || true
  case "$version" in go*) ;; *) die "Unexpected Go version response: ${version:-<empty>}" ;; esac
...
main() {
  ...
  ensure_cmd git
  ensure_cmd make
  ensure_cmd gcc            # cgo: mattn/go-sqlite3 needs a C compiler at build time
